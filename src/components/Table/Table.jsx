...
export const Table = () => {
  ...
  useEffect(() => {
    async function fetchRegister() {
      ...
      setTotalGeral(
        response.data.reduce((sum, item) => sum + (item.entry - item.output), 0)
      );
    }
    fetchRegister();
            ...
            <tbody>
              {data.map((row) => (
                <tr key={row.registerID}>
                  ...
                  <td style={{ color: row.entry > 0 ? "green" : "#969CB2" }}>
                    R$ {formatNumber(row.entry)}
                  </td>
                  <td style={{ color: row.output > 0 ? "red" : "#969CB2" }}>
                    R$ {formatNumber(row.output)}
                  </td>
                  <td>{row.dataFormatada}</td>
                  <td
                    style={{ fontSize: "35px" }}
                    onClick={() => handleOpenModalUpdate(row.registerID)}
                  >
                    <FcEditImage />
                  </td>
                  <td onClick={() => handleOpenModalDelete(row.registerID)}>
                    <img src={menos} alt="Icone de deletar linha" />
                  </td>
                </tr>
              ))}
            </tbody>
        ...
        >
          <Form className="form">
            <div>
              ...
              <Field name="entry" type="number" placeholder="Entrada" />
              <span>
                {" "}
                <ErrorMessage name="entry" />
              </span>
              <Field name="output" type="number" placeholder="Saida" />
              <span>
      ...
      >
        <div className="buttonDelete ">
          ...
          <div>
            ...
            <button onClick={() => handleTransationDelete(cod)}>
              Confirmar
            </button>
          </div>
        </div>
